...
class AudioProcessor:
    def __init__(self):
        ...
        # Control ÚNICO (dBFS)
        self.crest_margin_db = 2.0   # margen típico voz/música
        self.level_ceiling_db = -6.0  # slider principal (techo del limitador)
        self.max_gain_db      = 24.0  # límite de subida del AGC (+18 dB máx)
        ...
        self.release_a = db_to_alpha(0.30, self.fs, self.blocksize)
    ...
    def _callback(self, indata, outdata, frames, time_info, status):
        ...
        # --- Objetivo RMS (en dBFS) y en lineal ---
        target_rms_db = ceiling_db - crest_margin
        target_rms_lin = dbfs_to_lin(target_rms_db)

        # --- Ganancia deseada para alcanzar el objetivo ---
        desired_gain_lin = target_rms_lin / rms_in  # puede subir o bajar

        # ====== NOISE GATE (CONGELA SUBIDAS EN SEÑALES MUY BAJAS) ======
        noise_gate_db = -55.0  # ajusta a gusto: -55/-60 dBFS típico
        if rms_in_db < noise_gate_db:
            desired_gain_lin = min(desired_gain_lin, g_prev)
        # ===============================================================

        # --- Limitar ganancia máxima permitida (en dB) ---
        ...
        # --- Suavizado ataque/recuperación ---
        a = self.attack_a if desired_gain_lin < g_prev else self.release_a
        g = a * g_prev + (1 - a) * desired_gain_lin

        # --- Aplicar AGC ---
        y = x * g

        # --- Limitador a techo en dBFS ---
        peak = float(np.max(np.abs(y)))
        peak_db = lin_to_dbfs(peak) if peak > 1e-12 else -120.0
        if peak_db > ceiling_db:
            reduce_db = ceiling_db - peak_db  # negativo
            y *= dbfs_to_lin(reduce_db)

        # --- Soft-clip de seguridad y master ---
...
class Main(QtWidgets.QWidget):
    def __init__(self):
        ...
        if HAS_PG:
            ...
            # >>> COLORES: RMS ROJO, PEAK AZUL <<<
            self.curve_rms  = self.plot.plot(pen=pg.mkPen(color=(255, 0, 0), width=2))  # rojo
            self.curve_peak = self.plot.plot(pen=pg.mkPen(color=(0, 120, 255), width=1, style=QtCore.Qt.DashLine))  # azul
